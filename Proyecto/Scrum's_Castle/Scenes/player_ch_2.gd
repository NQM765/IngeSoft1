...
@export var final_destination: Vector2 = Vector2(482, 23 )  # Punto específico donde debe llegar y quedarse
const SPEED = 200
...
func _physics_process(delta):
	...
	# Lógica de animación basada en la selección de personaje y estado de la tarea
	var anim_name = ""
	
	# Si el estado de la tarea es "Fallido", se reproduce la animación de muerte
	if Datos.estados_tareas[1] == "Fallido":
		print("Estado tarea es Fallido")
		if Datos.personajes_seleccionados[1] == "Cazadora":
			anim_name = "DeathC"
		elif Datos.personajes_seleccionados[1] == "Mago":
			anim_name = "DeathM"
		elif Datos.personajes_seleccionados[1] == "Granjero":
			anim_name = "DeathG"
		elif Datos.personajes_seleccionados[1] == "":
			anim_name = "DeathK"
	else:
		...
		if velocity.length() < 1:
			if Datos.personajes_seleccionados[1] == "Cazadora":
				...
				anim_name = "IdleG"
			elif Datos.personajes_seleccionados[1] == "Caballero":
				anim_name = "IdleK"
		else:
...
func _on_nav_finished():
	...
	# Aquí puedes agregar lógica adicional si deseas, por ejemplo, reproducir una animación final o detener otras acciones.
